...
一个自用 Android 小工具：按你选择的文件夹自动清理过期图片，包括 **Download** 目录。纯本地运行，**没有任何网络权限**。

## 功能

- 🧹 **按目录自动清理** —— 可设置保留天数（1–60 天）和每天运行时间，在设置中添加文件夹并选择要自动清理的目录，每天后台检查一次
...
- 📥 **下载目录** —— Download 里的图片会和其他图片一起检测、展示和自动清理
- 🔖 **保留图片** —— 图片会移动到专用保留目录，不再出现在清理列表里；可在设置里管理并恢复
...
| 权限 | 用途 |
| --- | --- |
| 所有文件访问 / 存储读写 | 扫描并删除已选择的图片目录（包括 Download） |
