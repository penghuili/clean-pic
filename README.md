...
- 🧹 **按目录自动清理** —— 可设置保留天数（3–7 天）和每天运行时间，在设置中添加文件夹并选择要自动清理的目录，每天按时检查一次，并显示上次执行时间与下次运行时间
...
- Android 12+ 建议在设置页允许「精确闹钟」并忽略电池优化，定时才会准确
...
| 闹钟与提醒（Android 12+） | 到点准时触发每日清理，未授权时系统会延后执行 |
| 忽略电池优化 | 避免后台任务被系统省电策略清理掉 |
