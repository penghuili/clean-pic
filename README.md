...
- 🧹 **按目录自动清理** —— 可设置保留天数（3–7 天）和每天运行时间，在设置中添加文件夹并选择要自动清理的目录，每天后台检查一次，并显示上次后台执行时间
...
> Android 后台任务由 WorkManager 调度，07:05 是目标时间，系统可能因省电策略延后执行；设置页的“上次自动清理”时间可用来确认任务是否实际被唤起。
